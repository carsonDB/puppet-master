...
function buildTags (tag, cxt, gate)
{
	...
	if (tag.hasOwnProperty('children'))
		for (let i = 0; i < tag.children.length; i++)
		{
			...
			childNode.parent = dom;
			nUtil.appendNativeChild(dom, childNode);
		}
	return dom;
}
...
// puppet-dom manipulation
ProtoDom.prototype.add = function (obj)
{
	var dom;
	if (typeof obj == 'string')
		dom = this.create(obj);
	else if (obj.parent instanceof ProtoDom)
		dom = obj.remove();
	else
		dom = obj;
	this.children.push(dom);
	nUtil.appendNativeChild(this, dom);
};
ProtoDom.prototype.remove = function () 
{
	nUtil.removeNativeChild(this.parent, this);
	var list = this.parent.children;
	return list.splice(list.indexOf(this), 1)[0];
};
